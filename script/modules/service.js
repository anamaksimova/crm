...
export const URLS = 'http://localhost:3001/';
...
export const addItemData = async (item) => {
  const fetchPOST = await fetch(`${URLS}api/goods`, {
    method: 'POST',
    body: JSON.stringify({
      title: item.title,
      price: item.price,
...
export const loadGoods = async () => {
  const result = await fetch(`${URLS}api/goods`);
...
export const loadCategories = async () => {
  const result = await fetch(`${URLS}api/category`);
